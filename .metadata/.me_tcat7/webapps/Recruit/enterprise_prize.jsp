...
<html>
  ...
  <body>
	...
	<!-- 编辑窗口 -->
	<div id="win" class="easyui-window" title="奖品管理" style="width:500px;height:300px"   
        data-options="iconCls:'icon-save',modal:true,closed:true">   
	    <form id="editForm" method="post">
			<%--提供id隐藏域 --%>
			<input type="hidden" name="prize_id"> 
			             奖品名称：<input type="text" name="prize_name" class="easyui-textbox"
					data-options="required:true" /><br /> 
			            奖品数量：<input type="text" name="prize_amount" class="easyui-textbox"
	...
	<div id="winn" class="easyui-window" title="查看奖品信息" style="width:500px;height:300px"   
        data-options="iconCls:'icon-save',modal:true,closed:true">   
        <form id="searchForm" method="post">
			...
			<input type="hidden" name="prize_id"> 
					...
					data-options="required:false" /><br /> 
			              奖品数量：<input type="text" name="prize_amount" class="easyui-textbox"
					data-options="required:false" /><br />
				获奖人    ：<input type="text" name="user_id" class="easyui-textbox"
  	...
  	<script type="text/javascript">
  		$(function(){
  			$("#list").datagrid({
  				//url:后台数据查询的地址
  				url:"customer/listPrizeByPage.action",
  				//columns：填充的列数据
  					//field:后台对象的属性
  					//tille:列标题
  				columns:[[
  			...
  			$("#searchBtn").click(function(){
  				...
  				//表单回显
  				$("#searchForm").form("load","customer/findPrizeById.action?id="+rows[0].prize_id);
  			...
  			$("#editBtn").click(function(){
  				...
  				//表单回显
  				$("#editForm").form("load","customer/findPrizeById.action?id="+rows[0].prize_id);
  				...
  				$.messager.confirm("提示","确认删除数据吗?",function(value){
  					if(value){
  						...
  						$(rows).each(function(i){
  							idStr+=("id="+rows[i].prize_id+"&");
  						});
  						idStr = idStr.substring(0,idStr.length-1);
						
  						//传递到后台
  						$.post("customer/deletePrize.action",idStr,function(data){
  							if(data.success){
  	  							//刷新datagrid
  	  							$("#list").datagrid("reload");
